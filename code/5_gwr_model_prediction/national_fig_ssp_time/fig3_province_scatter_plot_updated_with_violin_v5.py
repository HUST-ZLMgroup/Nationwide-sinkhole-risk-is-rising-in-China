...
def resolve_metric_spec(df: pd.DataFrame) -> Dict[str, str]:
    """Resolve whether the input uses probability columns or raw-score columns."""
    if {"mean_prob_2020", "mean_prob_current"}.issubset(df.columns):
        return {
            "kind": "prob",
            "mean_base": "mean_prob_2020",
            "mean_future": "mean_prob_current",
            "median_base": "median_prob_2020",
            "median_future": "median_prob_current",
            "x_label": "Mean probability (2020)",
            "y_label_prefix": "Mean probability",
            "delta_label": "Δ mean probability",
        }

    if {"mean_score_2020", "mean_score_current"}.issubset(df.columns):
        return {
            "kind": "score",
            "mean_base": "mean_score_2020",
            "mean_future": "mean_score_current",
            "median_base": "median_score_2020",
            "median_future": "median_score_current",
            "x_label": "Mean raw GWR score (2020)",
            "y_label_prefix": "Mean raw GWR score",
            "delta_label": "Δ mean raw GWR score",
        }

    raise ValueError(
        "Missing columns in CSV: expected either "
        "['mean_prob_2020', 'mean_prob_current'] or "
        "['mean_score_2020', 'mean_score_current']"
    )
...
def make_fig3_scatter_only(
    ...
    metric = resolve_metric_spec(df)
    required = [metric["mean_base"], metric["mean_future"], "delta_mean"]
    ...
    x = d[metric["mean_base"]].to_numpy(dtype=float)
    y = d[metric["mean_future"]].to_numpy(dtype=float)
    ...
    if finite.any():
        raw_min = float(min(np.nanmin(x[finite]), np.nanmin(y[finite])))
        raw_max = float(max(np.nanmax(x[finite]), np.nanmax(y[finite])))
    else:
        raw_min, raw_max = 0.0, 0.1

    span = raw_max - raw_min
    pad = max(span * 0.05, 1.0e-3)

    if metric["kind"] == "prob":
        lim_min = max(0.0, raw_min - pad)
        lim_max = max(raw_max + pad, 0.1)
    else:
        lim_min = raw_min - pad
        lim_max = raw_max + pad
        if lim_min == lim_max:
    ...
    ax.set_xlabel(metric["x_label"], fontproperties=TNR, color=TEXT_BLACK)
    ax.set_ylabel(f"{metric['y_label_prefix']} ({year_disp})", fontproperties=TNR, color=TEXT_BLACK)
    ...
    # Nudge the scenario label ~1 mm left and ~2 mm up on the 6 cm canvas.
    ax.text(
        0.963,
        0.223,
...
def make_fig3_violin_only(
    ...
    if value_col == "delta_mean":
        ax.set_ylabel(metric["delta_label"], fontproperties=TNR, color=TEXT_BLACK)
    else:
...
def resolve_input_path(in_path: Optional[str] = None, out_dir: Optional[str] = None) -> str:
    ...
    candidates: List[str] = []
    patterns = [
        "province_raw_gwr_change_vs2020_lonlat_align_sortByChangeRate_*_*.csv",
        "province_prob_change_vs2020_lonlat_align_clip01_sortByChangeRate_*_*.csv",
    ]
    ...
    if not candidates:
        raise FileNotFoundError(
            "No matching province_raw_gwr_change...csv or province_prob_change...csv found. "
...
def make_average_df(in_paths: Sequence[str]) -> pd.DataFrame:
    ...
    The average is computed per-province (aligned by a detected join key if possible):
      - mean_*_2020: mean across inputs
      - mean_*_current: mean across inputs
      - delta_mean: recomputed as (mean_current - mean_2020)
    ...
    metric_specs = [resolve_metric_spec(df) for df in dfs]
    metric_kinds = {spec["kind"] for spec in metric_specs}
    if len(metric_kinds) != 1:
        raise ValueError(
            "Mixed metric kinds across inputs. Please do not mix probability CSVs "
            "with raw-score CSVs in one average plot."
        )
    metric = metric_specs[0]
    mean_base_col = metric["mean_base"]
    mean_future_col = metric["mean_future"]
    ...
    if join_key and all(join_key in df.columns for df in dfs):
        ...
        for df in dfs:
            ...
            s2020 = pd.to_numeric(g[mean_base_col].mean(), errors="coerce").reindex(common_ids)
            scur = pd.to_numeric(g[mean_future_col].mean(), errors="coerce").reindex(common_ids)
        ...
        out[mean_base_col] = m2020.mean(axis=1, skipna=True)
        out[mean_future_col] = mcur.mean(axis=1, skipna=True)
        out["delta_mean"] = out[mean_future_col] - out[mean_base_col]

        if npts_ref is not None:
            out["n_points"] = npts_ref

        with np.errstate(divide="ignore", invalid="ignore"):
            out["change_rate"] = out["delta_mean"] / out[mean_base_col]
    ...
    m2020 = np.vstack([_col_numeric(df, mean_base_col).to_numpy(dtype=float) for df in dfs])
    mcur = np.vstack([_col_numeric(df, mean_future_col).to_numpy(dtype=float) for df in dfs])

    out = pd.DataFrame()
    out[mean_base_col] = np.nanmean(m2020, axis=0)
    out[mean_future_col] = np.nanmean(mcur, axis=0)
    out["delta_mean"] = out[mean_future_col] - out[mean_base_col]
