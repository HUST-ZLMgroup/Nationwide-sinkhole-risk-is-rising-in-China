...
def _extract_tas_for_year(nc_path, sinkhole_position, label=None):
    """Extracts the average temperature for the year (averaged over the time dimension) for each point from the given NetCDF file.
    ...
    if not os.path.exists(nc_path):
        raise FileNotFoundError(f"[Tas] NetCDF :{nc_path}")

    print(f"[Tas] Open{label or os.path.basename(nc_path)}: {nc_path}")

    ds = xr.open_dataset(nc_path)
    try:
        # 1. Select temperature variable: use 'tas' first
        if "tas" in ds.data_vars:
            da = ds["tas"]
        else:
            # If there is no tas, take the first variable (usually this will not happen, just a cover-up)
            first_var = list(ds.data_vars)[0]
            da = ds[first_var]
            print(f"[Tas] Warning: Variable 'tas' not found, using{first_var}")
        ...
        if lat_name is None or lon_name is None:
            raise ValueError(
                "[Tas] Unable to identify latitude/longitude dimensions in NetCDF, please check file structure."
            )
...
def tas(
    ...
    """Calculate temperature (annual average) and take multi-year monthly averages over historical/future time periods.
    ...
    - Examples of historical data before 2015 (excluding 2015):
      Z:\\jing\\Large_scale\\future_dataset\\20_tas\\historical\\
      tas_day_BCC-CSM2-MR_historical_r1i1p1f1_gn_2000_v2.0.nc

      Assuming other year filename patterns:
      tas_day_BCC-CSM2-MR_historical_r1i1p1f1_gn_{year}_v2.0.nc

    - Examples of SSP data for 2015 and later (inclusive):
      Z:\\jing\\Large_scale\\future_dataset\\20_tas\\sspxxx\\
      tas_day_BCC-CSM2-MR_sspxxx_r1i1p1f1_gn_2017_v2.0.nc

      Assuming other year filename patterns:
      tas_day_BCC-CSM2-MR_{sspxxx}_r1i1p1f1_gn_{year}_v2.0.nc
    ...
    Calculation rules
    --------
    1) For each year file, first average the time dimension to obtain the annual average temperature field of that year.

    2) Historical data:
       Use the annual average temperatures in 2000, 2010, and 2020, and then average:
    ...
    3) Future data:
       Taking 20 years as a time period, using the annual average temperature in 10-year intervals, the three-year average:
       ...
       Tas_2020_2040, Tas_2040_2060, Tas_2060_2080, Tas_2080_2100
    ...
    Description
    ----
    If the original tas unit is K/Kelvin, the result will be automatically converted to ℃;
    If the original unit is already ℃ or other units, keep the original value."""

    print("\\n[Tas] Start calculating temperature...")

    # ---------------- 1. Check the input column ----------------
    required_cols = ["No", "Longitude", "Latitude"]
    for col in required_cols:
        if col not in sinkhole_position.columns:
            raise ValueError(f"[Tas] Input sinkhole_position is missing a required column: '{col}'")
    ...
    if ssp_key not in ssp_to_sspxxx:
        raise ValueError(
            f"[Tas] Unsupported SSP scenarios:{ssp}, currently only supports{list(ssp_to_sspxxx.keys())}"
        )

    sspxxx = ssp_to_sspxxx[ssp_key]  # Such as 'ssp3' -> 'ssp370'

    # ---------------- 3. Construct the data path root directory ----------------
    tas_root = os.path.join(
        database_folder_path,
        "20_tas",
    )

    # Directory where historical nc is located
    hist_dir = os.path.join(tas_root, "historical")

    # SSP nc directory:.../sspxxx/
    ssp_dir = os.path.join(tas_root, sspxxx)

    print(f"[Tas] Historical data directory:{hist_dir}")
    print(f"[Tas] SSP projection data directory:{ssp_dir}")
    print(f"[Tas] SSP :{ssp} -> {sspxxx}")
    ...
    future_windows = {
        "Tas_2020_2040": [2020, 2030, 2040],
        "Tas_2040_2060": [2040, 2050, 2060],
        "Tas_2060_2080": [2060, 2070, 2080],
        "Tas_2080_2100": [2080, 2090, 2100],
    }
    ...
    # ---------------- 5. Extract the annual average temperature for each year ----------------
    year_values = {}
    ...
    for year in years_needed:
        if year < 2015:
            ...
            # tas_day_BCC-CSM2-MR_historical_r1i1p1f1_gn_{year}_v2.0.nc
            nc_name = f"tas_day_BCC-CSM2-MR_historical_r1i1p1f1_gn_{year}_v2.0.nc"
            nc_path = os.path.join(hist_dir, nc_name)
        else:
            # Using SSP files
            # tas_day_BCC-CSM2-MR_{sspxxx}_r1i1p1f1_gn_{year}_v2.0.nc
            nc_name = f"tas_day_BCC-CSM2-MR_{sspxxx}_r1i1p1f1_gn_{year}_v2.0.nc"
            nc_path = os.path.join(ssp_dir, nc_name)

        label = f"{year}"
        year_values[year] = _extract_tas_for_year(
            nc_path, sinkhole_position, label=label
        )

    # ---------------- 6. History: 2000, 2010, 2020 average again ----------------
    print("[Tas] Calculate historical average (2000, 2010, 2020)...")
    ...
    for col_name, years in future_windows.items():
        print(f"[Tas]{col_name}Corresponding year{years}...")
    ...
    hist_output_path = os.path.join(
        historical_folder_path, "Tas_historical_2000_2010_2020.csv"
    )
    ...
    future_output_path = os.path.join(
        future_ssp_folder_path, f"Tas_future_{ssp}.csv"
    )
    future_df.to_csv(future_output_path, index=False, encoding="utf-8-sig")

    # ---------------- 9. Print statistical information ----------------
    print("\\n[Tas] Historical data statistics (average annual temperature in 2000, 2010, 2020):")
    ...
    print("\\n[Tas] Statistics for each future time period (according to SSP scenario):")
    ...
    print("\\n[Tas] Historical results have been saved to:")
    print("  ", hist_output_path)
    print("[Tas] Future results saved to:")
    print("  ", future_output_path)
    print("\\n[Tas] Result preview (history part):")
    print(hist_df.head())
    print("\\n[Tas] Result preview (future part):")
