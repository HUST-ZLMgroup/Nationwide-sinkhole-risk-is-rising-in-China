...
def _extract_tas_for_year(nc_path, sinkhole_position, label=None):
    """
    从给定的 NetCDF 文件中提取每个点该年的平均气温（对 time 维度取平均）。

    返回值为长度与 sinkhole_position 相同的一维 numpy 数组。
    若变量单位为 K / Kelvin，则转换为 ℃。
    """
    if not os.path.exists(nc_path):
        raise FileNotFoundError(f"[Tas] 未找到 NetCDF 文件: {nc_path}")

    print(f"[Tas] 打开 {label or os.path.basename(nc_path)}: {nc_path}")
    ...
    try:
        ...
        if "tas" in ds.data_vars:
            ...
            # 如果没有 tas，就取第一个变量（一般不会发生，只是兜底）
            first_var = list(ds.data_vars)[0]
            da = ds[first_var]
            print(f"[Tas] 警告: 未找到变量 'tas'，使用 {first_var}")
        ...
        if lat_name is None or lon_name is None:
            raise ValueError(
                "[Tas] 无法在 NetCDF 中识别纬度/经度维度，请检查文件结构。"
            )
...
def tas(
    ...
    计算规则
    --------
    1) 对每个年文件，先对 time 维进行平均，得到该年的年平均气温场。

    2) 历史数据：
       使用 2000, 2010, 2020 三个年份的年平均气温，再求平均：
    ...
    3) 未来数据：
       以 20 年为时间段，使用 10 年间隔年份的年平均气温，三年平均：
    ...
    for col in required_cols:
        if col not in sinkhole_position.columns:
            raise ValueError(f"[Tas] 输入的 sinkhole_position 缺少必要列: '{col}'")
    ...
    if ssp_key not in ssp_to_sspxxx:
        raise ValueError(
            f"[Tas] 不支持的 SSP 情景: {ssp}，当前只支持 {list(ssp_to_sspxxx.keys())}"
        )
    ...
    print(f"[Tas] 历史数据目录: {hist_dir}")
    print(f"[Tas] SSP 投影数据目录: {ssp_dir}")
    print(f"[Tas] 当前 SSP 情景: {ssp} -> {sspxxx}")
    ...
    print("[Tas] 计算历史平均（2000, 2010, 2020）...")
    ...
    for col_name, years in future_windows.items():
        print(f"[Tas] 计算未来时间段 {col_name} 对应年份 {years} 的年平均气温平均...")
    ...
    # ---------------- 9. 打印统计信息 ----------------
    print("\n[Tas] 历史数据统计（2000, 2010, 2020 年均气温的平均）:")
