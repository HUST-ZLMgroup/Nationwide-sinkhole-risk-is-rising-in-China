...
def _extract_huss_for_year(nc_path, sinkhole_position, label=None):
    """Extracts the average specific humidity for that year (averaged over the time dimension) for each point from the given NetCDF file.

    The return value is a one-dimensional numpy array with the same length as sinkhole_position.
    The common unit of huss is kg/kg (dimensionless mass ratio), and the original unit is kept unchanged by default."""
    if not os.path.exists(nc_path):
        raise FileNotFoundError(f"[Huss] NetCDF file not found:{nc_path}")

    print(f"[Huss] Open{label or os.path.basename(nc_path)}: {nc_path}")

    ds = xr.open_dataset(nc_path)
    try:
        # 1. Select specific humidity variable: use 'huss' first
        if "huss" in ds.data_vars:
            da = ds["huss"]
        else:
            # If there is no huss, just take the first variable (usually it won’t happen, it’s just a cover-up)
            first_var = list(ds.data_vars)[0]
            da = ds[first_var]
            print(f"[Huss] Warning: Variable 'huss' not found, use{first_var}")
        ...
        if lat_name is None or lon_name is None:
            raise ValueError(
                "[Huss] Unable to identify latitude/longitude dimensions in NetCDF, please check file structure."
            )
...
def huss(
    ...
    """Calculate specific humidity (annual average) and take multi-year monthly averages over historical/future time periods.
    ...
    - Examples of historical data before 2015 (excluding 2015):
      Z:\\jing\\Large_scale\\future_dataset\\23_huss\\historical\\
      huss_day_BCC-CSM2-MR_historical_r1i1p1f1_gn_2000_v2.0.nc

      Assuming other year filename patterns:
      huss_day_BCC-CSM2-MR_historical_r1i1p1f1_gn_{year}_v2.0.nc

    - Examples of SSP data for 2015 and later (inclusive):
      Z:\\jing\\Large_scale\\future_dataset\\23_huss\\sspxxx\\
      huss_day_BCC-CSM2-MR_sspxxx_r1i1p1f1_gn_2017_v2.0.nc

      Assuming other year filename patterns:
      huss_day_BCC-CSM2-MR_{sspxxx}_r1i1p1f1_gn_{year}_v2.0.nc
    ...
    Calculation rules
    --------
    1) For each year file, first average the time dimension to obtain the annual average specific humidity field of that year.

    2) Historical data:
       Use the annual average specific humidity in 2000, 2010, and 2020, and then average it:
    ...
    3) Future data:
       Taking 20 years as a time period, using the annual average specific humidity of 10-year intervals, the three-year average:
       ...
       Huss_2020_2040, Huss_2040_2060, Huss_2060_2080, Huss_2080_2100
    ...
    Description
    ----
    The common unit of huss is kg/kg (dimensionless mass ratio). Unit conversion is not performed by default here.
    Keep the original data units directly."""

    print("\\n[Huss] Start calculating specific humidity...")

    # ---------------- 1. Check the input column ----------------
    required_cols = ["No", "Longitude", "Latitude"]
    for col in required_cols:
        if col not in sinkhole_position.columns:
            raise ValueError(f"[Huss] The input sinkhole_position is missing a required column: '{col}'")
    ...
    if ssp_key not in ssp_to_sspxxx:
        raise ValueError(
            f"[Huss] Unsupported SSP scenarios:{ssp}, currently only supports{list(ssp_to_sspxxx.keys())}"
        )

    sspxxx = ssp_to_sspxxx[ssp_key]

    # ---------------- 3. Construct the data path root directory ----------------
    huss_root = os.path.join(
        database_folder_path,
        "23_huss",
    )

    hist_dir = os.path.join(huss_root, "historical")
    ssp_dir = os.path.join(huss_root, sspxxx)

    print(f"[Huss] :{hist_dir}")
    print(f"[Huss] SSP projection data directory:{ssp_dir}")
    print(f"[Huss] Current SSP scenario:{ssp} -> {sspxxx}")

    # ---------------- 4. ----------------
    hist_years = [2000, 2010, 2020]

    future_windows = {
        "Huss_2020_2040": [2020, 2030, 2040],
        "Huss_2040_2060": [2040, 2050, 2060],
        "Huss_2060_2080": [2060, 2070, 2080],
        "Huss_2080_2100": [2080, 2090, 2100],
    }
    ...
    for year in years_needed:
        if year < 2015:
            nc_name = f"huss_day_BCC-CSM2-MR_historical_r1i1p1f1_gn_{year}_v2.0.nc"
            nc_path = os.path.join(hist_dir, nc_name)
        else:
            nc_name = f"huss_day_BCC-CSM2-MR_{sspxxx}_r1i1p1f1_gn_{year}_v2.0.nc"
            nc_path = os.path.join(ssp_dir, nc_name)

        label = f"{year}"
        year_values[year] = _extract_huss_for_year(
            nc_path, sinkhole_position, label=label
        )

    # ---------------- 6. History: 2000, 2010, 2020 average again ----------------
    print("[Huss] Calculate historical average (2000, 2010, 2020)...")
    ...
    for col_name, years in future_windows.items():
        print(f"[Huss] Calculate future time period{col_name}Corresponding year{years}’s annual average specific humidity average...")
    ...
    hist_output_path = os.path.join(
        historical_folder_path, "Huss_historical_2000_2010_2020.csv"
    )
    ...
    future_output_path = os.path.join(
        future_ssp_folder_path, f"Huss_future_{ssp}.csv"
    )
    future_df.to_csv(future_output_path, index=False, encoding="utf-8-sig")

    # ---------------- 9. Print statistical information ----------------
    print("\\n[Huss] Historical data statistics (average annual specific humidity in 2000, 2010, 2020):")
    if not hist_df[hist_col].isna().all():
        print(f"Points:{len(hist_df)}")
        print(f"Minimum value:{hist_df[hist_col].min():.6f}")
        print(f":{hist_df[hist_col].max():.6f}")
        print(f"Average:{hist_df[hist_col].mean():.6f}")
    ...
    for col_name in future_windows.keys():
        ...
        print(f":{col_series.min():.6f}")
        print(f"Maximum value:{col_series.max():.6f}")
        print(f"Average:{col_series.mean():.6f}")

    print("\\n[Huss] Historical results have been saved to:")
    print("  ", hist_output_path)
    print("[Huss] Future results saved to:")
    print("  ", future_output_path)
    print("\\n[Huss] Result preview (history part):")
    print(hist_df.head())
    print("\\n[Huss] Results preview (future part):")
