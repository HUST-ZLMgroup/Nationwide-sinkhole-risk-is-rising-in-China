...
def _extract_huss_for_year(nc_path, sinkhole_position, label=None):
    """
    从给定的 NetCDF 文件中提取每个点该年的平均比湿（对 time 维度取平均）。

    返回值为长度与 sinkhole_position 相同的一维 numpy 数组。
    huss 常见单位为 kg/kg（无量纲质量比），此处默认保持原始单位不变。
    """
    if not os.path.exists(nc_path):
        raise FileNotFoundError(f"[Huss] 未找到 NetCDF 文件: {nc_path}")

    print(f"[Huss] 打开 {label or os.path.basename(nc_path)}: {nc_path}")
    ...
    try:
        ...
        if "huss" in ds.data_vars:
            ...
            # 如果没有 huss，就取第一个变量（一般不会发生，只是兜底）
            first_var = list(ds.data_vars)[0]
            da = ds[first_var]
            print(f"[Huss] 警告: 未找到变量 'huss'，使用 {first_var}")
        ...
        if lat_name is None or lon_name is None:
            raise ValueError(
                "[Huss] 无法在 NetCDF 中识别纬度/经度维度，请检查文件结构。"
            )
...
def huss(
    ...
    计算规则
    --------
    1) 对每个年文件，先对 time 维进行平均，得到该年的年平均比湿场。

    2) 历史数据：
       使用 2000, 2010, 2020 三个年份的年平均比湿，再求平均：
    ...
    3) 未来数据：
       以 20 年为时间段，使用 10 年间隔年份的年平均比湿，三年平均：
    ...
    说明
    ----
    huss 常见单位为 kg/kg（无量纲质量比），此处默认不做单位换算，
    直接保留原始数据单位。
    """
    ...
    for col in required_cols:
        if col not in sinkhole_position.columns:
            raise ValueError(f"[Huss] 输入的 sinkhole_position 缺少必要列: '{col}'")
    ...
    if ssp_key not in ssp_to_sspxxx:
        raise ValueError(
            f"[Huss] 不支持的 SSP 情景: {ssp}，当前只支持 {list(ssp_to_sspxxx.keys())}"
        )
    ...
    print(f"[Huss] 历史数据目录: {hist_dir}")
    print(f"[Huss] SSP 投影数据目录: {ssp_dir}")
    print(f"[Huss] 当前 SSP 情景: {ssp} -> {sspxxx}")
    ...
    print("[Huss] 计算历史平均（2000, 2010, 2020）...")
    ...
    for col_name, years in future_windows.items():
        print(f"[Huss] 计算未来时间段 {col_name} 对应年份 {years} 的年平均比湿平均...")
    ...
    # ---------------- 9. 打印统计信息 ----------------
    print("\n[Huss] 历史数据统计（2000, 2010, 2020 年均比湿的平均）:")
    if not hist_df[hist_col].isna().all():
        print(f"  点数: {len(hist_df)}")
        print(f"  最小值: {hist_df[hist_col].min():.6f}")
        print(f"  最大值: {hist_df[hist_col].max():.6f}")
        print(f"  平均值: {hist_df[hist_col].mean():.6f}")

    print("\n[Huss] 未来各时间段统计（按 SSP 情景）:")
    for col_name in future_windows.keys():
        ...
        print(f"    最小值: {col_series.min():.6f}")
        print(f"    最大值: {col_series.max():.6f}")
        print(f"    平均值: {col_series.mean():.6f}")
