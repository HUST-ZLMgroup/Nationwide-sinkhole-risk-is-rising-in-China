...
def _extract_tasmin_for_year(nc_path, sinkhole_position, label=None):
    """
    从给定的 NetCDF 文件中提取每个点该年的平均最低气温（对 time 维度取平均）。

    返回值为长度与 sinkhole_position 相同的一维 numpy 数组。
    若变量单位为 K / Kelvin，则转换为 ℃。
    """
    if not os.path.exists(nc_path):
        raise FileNotFoundError(f"[Tasmin] 未找到 NetCDF 文件: {nc_path}")

    print(f"[Tasmin] 打开 {label or os.path.basename(nc_path)}: {nc_path}")

    ds = xr.open_dataset(nc_path)
    try:
        # 1. 选择最低气温变量：优先使用 'tasmin'
        if "tasmin" in ds.data_vars:
            da = ds["tasmin"]
        else:
            # 如果没有 tasmin，就取第一个变量（一般不会发生，只是兜底）
            first_var = list(ds.data_vars)[0]
            da = ds[first_var]
            print(f"[Tasmin] 警告: 未找到变量 'tasmin'，使用 {first_var}")
        ...
        if lat_name is None or lon_name is None:
            raise ValueError(
                "[Tasmin] 无法在 NetCDF 中识别纬度/经度维度，请检查文件结构。"
            )
...
def tasmin(
    ...
    计算规则
    --------
    1) 对每个年文件，先对 time 维进行平均，得到该年的年平均最低气温场。

    2) 历史数据：
       使用 2000, 2010, 2020 三个年份的年平均最低气温，再求平均：
    ...
    3) 未来数据：
       以 20 年为时间段，使用 10 年间隔年份的年平均最低气温，三年平均：
    ...
    说明
    ----
    若原始 tasmin 单位为 K / Kelvin，则结果自动转换为 ℃；
    若原始单位已是 ℃ 或其他单位，则保持原值。
    """
    ...
    for col in required_cols:
        if col not in sinkhole_position.columns:
            raise ValueError(f"[Tasmin] 输入的 sinkhole_position 缺少必要列: '{col}'")
    ...
    if ssp_key not in ssp_to_sspxxx:
        raise ValueError(
            f"[Tasmin] 不支持的 SSP 情景: {ssp}，当前只支持 {list(ssp_to_sspxxx.keys())}"
        )
    ...
    print(f"[Tasmin] 历史数据目录: {hist_dir}")
    print(f"[Tasmin] SSP 投影数据目录: {ssp_dir}")
    print(f"[Tasmin] 当前 SSP 情景: {ssp} -> {sspxxx}")
    ...
    print("[Tasmin] 计算历史平均（2000, 2010, 2020）...")
    ...
    for col_name, years in future_windows.items():
        print(f"[Tasmin] 计算未来时间段 {col_name} 对应年份 {years} 的年平均最低气温平均...")
    ...
    # ---------------- 9. 打印统计信息 ----------------
    print("\n[Tasmin] 历史数据统计（2000, 2010, 2020 年均最低气温的平均）:")
