...
def _extract_tasmin_for_year(nc_path, sinkhole_position, label=None):
    """NetCDF ( time ). sinkhole_position numpy . K / Kelvin, ℃."""
    if not os.path.exists(nc_path):
        raise FileNotFoundError(f"[Tasmin] NetCDF file not found:{nc_path}")

    print(f"[Tasmin]{label or os.path.basename(nc_path)}: {nc_path}")

    ds = xr.open_dataset(nc_path)
    try:
        # 1. Select the minimum temperature variable: use 'tasmin' first
        if "tasmin" in ds.data_vars:
            da = ds["tasmin"]
        else:
            # If there is no tasmin, just take the first variable (generally it won’t happen, it’s just a cover-up)
            first_var = list(ds.data_vars)[0]
            da = ds[first_var]
            print(f"[Tasmin] warning: variable 'tasmin' not found, using{first_var}")
...
def tasmin(
    ...
    """Calculate minimum temperature (annual average) and take multi-year monthly averages over historical/future time periods.
    ...
    - Examples of historical data before 2015 (excluding 2015):
      Z:\\jing\\Large_scale\\future_dataset\\22_tasmin\\historical\\
      tasmin_day_BCC-CSM2-MR_historical_r1i1p1f1_gn_2000_v2.0.nc

      Assuming other year filename patterns:
      tasmin_day_BCC-CSM2-MR_historical_r1i1p1f1_gn_{year}_v2.0.nc

    - Examples of SSP data for 2015 and later (inclusive):
      Z:\\jing\\Large_scale\\future_dataset\\22_tasmin\\sspxxx\\
      tasmin_day_BCC-CSM2-MR_sspxxx_r1i1p1f1_gn_2017_v2.0.nc

      Assuming other year filename patterns:
      tasmin_day_BCC-CSM2-MR_{sspxxx}_r1i1p1f1_gn_{year}_v2.0.nc
    ...
    Calculation rules
    --------
    1) For each year file, first average the time dimension to obtain the annual average minimum temperature field of that year.

    2) Historical data:
       Use the annual average minimum temperatures in 2000, 2010, and 2020, and then average:
    ...
    3) Future data:
       Taking 20 years as a time period, using the annual average minimum temperature in 10-year intervals, the three-year average:
       ...
       Tasmin_2020_2040, Tasmin_2040_2060, Tasmin_2060_2080, Tasmin_2080_2100
    ...
    Description
    ----
    If the original tasmin unit is K/Kelvin, the result is automatically converted to ℃;
    ...
    for col in required_cols:
        if col not in sinkhole_position.columns:
            raise ValueError(f"[Tasmin] sinkhole_position : '{col}'")
    ...
    if ssp_key not in ssp_to_sspxxx:
        raise ValueError(
            f"[Tasmin] Unsupported SSP scenarios:{ssp}, currently only supports{list(ssp_to_sspxxx.keys())}"
        )

    sspxxx = ssp_to_sspxxx[ssp_key]

    # ---------------- 3. Construct the data path root directory ----------------
    tasmin_root = os.path.join(
        database_folder_path,
        "22_tasmin",
    )

    hist_dir = os.path.join(tasmin_root, "historical")
    ssp_dir = os.path.join(tasmin_root, sspxxx)

    print(f"[Tasmin] Historical data directory:{hist_dir}")
    print(f"[Tasmin] SSP projection data directory:{ssp_dir}")
    print(f"[Tasmin] SSP :{ssp} -> {sspxxx}")

    # ---------------- 4. ----------------
    hist_years = [2000, 2010, 2020]

    future_windows = {
        "Tasmin_2020_2040": [2020, 2030, 2040],
        "Tasmin_2040_2060": [2040, 2050, 2060],
        "Tasmin_2060_2080": [2060, 2070, 2080],
        "Tasmin_2080_2100": [2080, 2090, 2100],
    }
    ...
    # ---------------- 5. Extract the annual average minimum temperature for each year ----------------
    year_values = {}

    for year in years_needed:
        if year < 2015:
            nc_name = f"tasmin_day_BCC-CSM2-MR_historical_r1i1p1f1_gn_{year}_v2.0.nc"
            nc_path = os.path.join(hist_dir, nc_name)
        else:
            nc_name = f"tasmin_day_BCC-CSM2-MR_{sspxxx}_r1i1p1f1_gn_{year}_v2.0.nc"
            nc_path = os.path.join(ssp_dir, nc_name)

        label = f"{year}"
        year_values[year] = _extract_tasmin_for_year(
            nc_path, sinkhole_position, label=label
        )

    # ---------------- 6. History: 2000, 2010, 2020 average again ----------------
    print("[Tasmin] Calculate historical average (2000, 2010, 2020)...")
    ...
    for col_name, years in future_windows.items():
        print(f"[Tasmin]{col_name}Corresponding year{years}’s average annual minimum temperature average...")
    ...
    hist_output_path = os.path.join(
        historical_folder_path, "Tasmin_historical_2000_2010_2020.csv"
    )
    ...
    print("\\n[Tasmin] Historical results have been saved to:")
    print("  ", hist_output_path)
    print("[Tasmin] Future results saved to:")
    print("  ", future_output_path)
    print("\\n[Tasmin] Result preview (history part):")
    print(hist_df.head())
    print("\\n[Tasmin] Result preview (future part):")
