...
def _extract_pop_values_for_year(tif_path, sinkhole_position, year_label=None):
    """
    从给定的 GeoTIFF 中按点提取人口总数像元值，返回一个长度与 sinkhole_position 相同的 numpy 数组。
    """
    if not os.path.exists(tif_path):
        raise FileNotFoundError(f"[Population_total] 未找到栅格文件: {tif_path}")
...
def population_total(
    ...
    """
    提取人口总数（total population）并计算历史/未来时间段平均值。

    数据说明
    --------
    - 2000 年基准年：
      Z:\\jing\\Large_scale\\future_dataset\\5_population_counts_(for_total_urban_rural_populations)\\total\\GeoTIFF_BaseYear_total\\baseYr_total_2000.tif

    - 2010-2100 年 SSP 投影：
      Z:\\jing\\Large_scale\\future_dataset\\5_population_counts_(for_total_urban_rural_populations)\\total\\SSPx\\sspx_total_2xxx.tif
      其中 SSPx 对应 SSP1/SSP2/...，sspx 对应 ssp1/ssp2/...，2xxx 为年份（2010-2100，10 年间隔）
    ...
    print("\n[Population_total] 开始计算人口总数（total population）...")
    ...
    for col in required_cols:
        if col not in sinkhole_position.columns:
            raise ValueError(
                f"[Population_total] 输入的 sinkhole_position 缺少必要列: '{col}'"
            )
    ...
    # 2010-2100 年 SSP 投影数据：.../total/SSPx/sspx_total_2010.tif
    ssp_folder_name = ssp.upper()  # 'ssp3' -> 'SSP3'
    proj_dir = os.path.join(pop_root, ssp_folder_name)

    print(f"[Population_total] BaseYear 2000 栅格路径: {base2000_path}")
    print(f"[Population_total] SSP 投影数据目录: {proj_dir}")
    print(f"[Population_total] 当前 SSP 情景: {ssp}")
    ...
    for year in years_needed:
        if year == 2000:
            ...
            # 例如：SSP3/ssp3_total_2010.tif
            tif_name = f"{ssp}_total_{year}.tif"  # ssp 为小写，如 'ssp3'
            tif_path = os.path.join(proj_dir, tif_name)

        print(f"[Population_total] 年份 {year} 使用的栅格: {tif_path}")
        year_values[year] = _extract_pop_values_for_year(
            tif_path, sinkhole_position, year_label=str(year)
        )

    # ---------------- 5. 历史：2000, 2010, 2020 平均 ----------------
    print("[Population_total] 计算历史平均（2000, 2010, 2020）...")
    ...
    for col_name, years in future_windows.items():
        print(f"[Population_total] 计算未来时间段 {col_name} 对应年份 {years} 的平均...")
    ...
    # ---------------- 8. 打印统计信息 ----------------
    print("\n[Population_total] 历史数据统计（2000, 2010, 2020 平均）:")
    ...
    print("\n[Population_total] 未来各时间段统计（按 SSP 情景）:")
    ...
    print("\n[Population_total] 历史结果已保存至:")
    print("  ", hist_output_path)
    print("[Population_total] 未来结果已保存至:")
    print("  ", future_output_path)
    print("\n[Population_total] 结果预览（历史部分）:")
    print(hist_df.head())
    print("\n[Population_total] 结果预览（未来部分）:")
