...
def make_average_df(in_paths: Sequence[str]) -> pd.DataFrame:
    ...
    if join_key and all(join_key in df.columns for df in dfs):
        # Work with string IDs consistently to avoid dtype mismatches across files
        dfs2: List[pd.DataFrame] = []
        for df in dfs:
            dfx = df.copy()
            dfx[join_key] = dfx[join_key].astype(str)
            dfs2.append(dfx)
        dfs = dfs2

        idx_series = [df[join_key] for df in dfs]
        ...
        def _series(df: pd.DataFrame, col: str) -> pd.Series:
            if col not in df.columns:
                return pd.Series(index=df[join_key], dtype=float)
            return pd.to_numeric(df[col], errors="coerce").set_axis(df[join_key])

        m2020 = pd.concat(
            [_series(df[df[join_key].isin(common_ids)], "mean_prob_2020") for df in dfs],
            axis=1,
        )
        mcur = pd.concat(
            [_series(df[df[join_key].isin(common_ids)], "mean_prob_current") for df in dfs],
            axis=1,
        )
        ...
        if "n_points" in dfs[0].columns:
            npts = pd.to_numeric(dfs[0].set_index(join_key)["n_points"], errors="coerce")
            out["n_points"] = npts.reindex(out.index)
